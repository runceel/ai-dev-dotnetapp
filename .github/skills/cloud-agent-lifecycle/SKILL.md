...
| E2E テスト | Tester が Playwright MCP server で実行し、結果を `e2e-report.md` に保存する |
...
| E2E テスト結果 | `.github/lifecycle/<work-id>/e2e-report.md` |
...
2. Developer は PR 本文案を出力し、Orchestrator は `pull-request.md` に保存する。
3. Reviewer がコードと docs 更新をレビューする。
4. Reviewer が APPROVE した後、Orchestrator は Tester に E2E テストを委譲する。
5. Tester は Playwright MCP server 由来のブラウザ操作ツールを使用して E2E テストを実行し、結果を `e2e-report.md` と `log.md` に記録する。
6. 実装、テスト、docs 更新、レビュー、E2E 結果を `log.md` と `state.yaml` に記録する。
7. Ready for Review、マージ、Issue コメント投稿は人間または別の write 権限を持つ仕組みが行う。

### Cloud Agent E2E テスト

Cloud Agent で `cloud-agent-lifecycle` を使用する場合、E2E テストは Tester の責務とする。Tester は `e2e-test` スキルと Tester agent の手順に従い、Playwright MCP server 由来のツールを最優先で使用する。

| 状況 | 扱い |
|------|------|
| Playwright MCP server が利用可能 | Tester が MCP ツールで E2E テストを実行する |
| MCP ツールで必要な操作を実行できない | Tester が理由を `e2e-report.md` に記録し、`ENV_FAILED` または `FAIL` を返す |
| MCP server が未提供 | 原則として `ENV_FAILED` とし、Orchestrator が `state.yaml` と `log.md` に記録する |

Cloud Agent では `playwright-cli` を通常の実行経路にしない。CLI フォールバックは、ユーザーが明示的に許可した場合、または実行環境に Playwright MCP server が提供されていないことを承知でフォールバックを求めた場合のみ使用する。

Tester の E2E 完了出力には、少なくとも以下を含める:

- 使用したツール（Playwright MCP server / フォールバック時のみ `playwright-cli`）
- 実行したシナリオ一覧
- PASS / FAIL / ENV_FAILED / FLAKY_TEST の判定
- スクリーンショットや追加成果物のパス
- フォールバックまたは未実行がある場合の理由
...
.github/lifecycle/issue-42/outbox/
  ...
  04-pr-body.md
  05-tester-step-3.7-e2e-result.md
```
