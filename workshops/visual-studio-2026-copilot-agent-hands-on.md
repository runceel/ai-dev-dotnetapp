...
イベント一覧画面では複数イベントの参加状況を同時に表示するため、1 イベント単位の既存 UseCase（`GetRegistrationsByEventUseCase`）をループで呼び出す実装にすると N+1 クエリになります。

このハンズオンでは、以下のいずれかの方針を Agent と相談しながら選択してください。

| 方針 | 内容 | 適しているケース |
|------|------|------------------|
| A. 既存 UseCase をループで呼ぶ | シンプルだが N+1 問題あり | 30 分ハンズオンで時間を優先する場合 |
| B. 一覧取得用の Repository メソッドを追加 | `GetConfirmedCountsByEventIdsAsync` のように複数件を 1 回でまとめて取得する | 設計の正しさを優先する場合 |

時間制約から本ハンズオンでは方針 A を許容しますが、**「想定リスク」として N+1 問題を計画に含めるよう Agent に指示してください**。
...
- 一覧画面では複数イベントを表示するため、N+1 問題のリスクを想定リスクに含めてください
...
計画には次を含めてください:
- 変更対象ファイル
- 設計方針（既存 UseCase をループする / 一覧取得用メソッドを追加する 等の選択と理由）
- テスト方針（IRegistrationRepository モック追加など、既存テストセットアップへの影響を含む）
- 想定リスク（N+1、テストセットアップの変更範囲 等）
```
...
| データ取得方針 | N+1 問題のリスクが「想定リスク」として明示されているか |
| モジュール選択 | Analytics モジュールではなく Registrations モジュールから取得しているか |
| テスト | bUnit テストの追加または更新が計画に含まれているか |
| テスト依存 | `IRegistrationRepository` または `GetRegistrationsByEventUseCase` のテストセットアップ追加が含まれているか |
| 過剰さ | 新しいプロジェクト、大きなリファクタリング、不要な抽象化が含まれていないか |

計画が大きすぎる場合は、次のプロンプトで小さくします。なお、本格的な計画レビューは次の Step 3 で行います。
...
Step 2 で出てきた Agent の計画を、人間が一度落ち着いて確認します。前掲の「計画をレビューする観点」テーブルを使って、変更範囲・設計・再利用・データ取得方針・モジュール選択・テスト・過剰さの各観点を点検してください。

問題が見つかった場合は、いきなり実装に進まず、次のような追加プロンプトで計画自体を小さくしてから次のステップに進みます。

```text
計画をもう少し小さくしてください。
イベント一覧画面と既存テストの更新を中心にしてください。
新しいプロジェクトや大きな抽象化は追加しない方針で再計画してください。
Analytics モジュールは使わず、Registrations モジュールから取得する方針を維持してください。
まだコードは変更しないでください。
```

計画が妥当だと判断できたら、Step 4 に進みます。
