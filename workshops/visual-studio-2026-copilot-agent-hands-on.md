# Visual Studio 2026 GitHub Copilot エージェント モード ハンズオン

このハンズオンでは、EventRegistration リポジトリを題材に、Visual Studio 2026 の GitHub Copilot エージェント モードで既存 .NET アプリケーションの調査、計画、実装、検証を進めます。

- **概要**: イベント一覧画面に参加確定者数、残り枠数、満席表示、キャンセル待ち表示を追加する
- **実施時間**: 30 分
- **前提環境**: Visual Studio 2026、GitHub Copilot エージェント モード、.NET 10 SDK

---

## このハンズオンで行うこと

このハンズオンでは、Visual Studio 2026 の GitHub Copilot エージェント モードを使って、初めて見る既存 .NET ソリューションに小さな機能追加を行います。

題材はイベント参加登録システムです。イベント一覧画面のカードに、参加確定者数、残り枠数、満席表示、キャンセル待ち有無を追加します。

この資料は、参加者が自分の Visual Studio 2026 上でそのまま手順を進められるように書かれています。
...
2. **ビルドとテストが通る状態にする** — ソリューションのビルドが成功し、テスト エクスプローラーで `EventRegistration.Web.Tests` のテストがすべて成功していることを確認する
...
重要なのは、エージェント モードにすべてを一度に任せないことです。調査、計画、実装、検証を分けて依頼すると、既存プロジェクトでも安全に変更を進めやすくなります。
...
- ソリューション エクスプローラーにソリューションと各プロジェクトが表示されている
- `EventRegistration.AppHost` をスタートアップ対象として選択できる
- Build メニューからソリューションのビルドを実行できる
- テスト エクスプローラーに `EventRegistration.Web.Tests` のテストが表示される

---

## アプリの起動と現状確認

ハンズオン開始前に、変更対象となる画面を実際に起動して確認しておきます。

1. ソリューション エクスプローラーで `Hosting/EventRegistration.AppHost` プロジェクトを右クリックし、スタートアップ プロジェクトに設定する
2. デバッグ実行する

Aspire ダッシュボードが開きます。

![Aspire ダッシュボード](./images/aspire-dashboard.png)

ダッシュボード上の赤枠で示した `web` リソースのエンドポイントをクリックすると、アプリの Web 画面が開きます。サイドメニューから **イベント管理** を選ぶと、今回のハンズオンで機能追加を行うイベント一覧画面が表示されます。

![ハンズオン対象のイベント一覧画面（変更前）](./images/before.png)

このカードに参加確定者数、残り枠数、満席表示、キャンセル待ち表示を加えていきます。
...
今回の変更を完了したときの画面イメージは次のとおりです。UI の細かい見た目はハンズオン中の Agent の判断によって変わりますが、満席バッジ、キャンセル待ち表示、残り枠数などが追加された状態になります。

![機能追加後のイベント一覧画面のイメージ](./images/after.png)
...
Visual Studio の GitHub Copilot Chat をエージェント モードに切り替え、次のプロンプトを貼り付けてください。
...
調査が終わったら Planning ツールを使って実装計画を作成してください。
まだコードは変更しないでください。
...
Visual Studio では、ビルド出力とテスト エクスプローラーでも結果を確認してください。
...
4. テスト エクスプローラーで `EventRegistration.Web.Tests` のテストを実行する
...
時間があれば、「アプリの起動と現状確認」と同じ手順でアプリを起動し、イベント一覧画面で次の表示を確認してください。アプリを起動したままにしている場合は、ブラウザを再読み込みするだけで確認できます。
...
エージェント モードは、いきなり全作業を任せるためだけの機能ではありません。
...
## おまけ: このリポジトリの Copilot カスタマイズ

このリポジトリには、ハンズオン本編で使った Visual Studio 2026 の GitHub Copilot エージェント モードに加えて、Copilot にプロジェクト固有の開発ルールや役割を伝えるためのカスタマイズが含まれています。

時間が余った場合や、ハンズオン後にさらに学びたい場合は、`.github` 配下の定義を読むと、Copilot をチーム開発向けにどう活用できるかの参考になります。

### カスタム指示

`.github/copilot-instructions.md` には、このリポジトリ全体で Copilot に守ってほしい開発方針が書かれています。

例えば、次のような内容です。

- 要件理解、仕様策定、開発、テストの進め方
- コード変更時に参照するスキル
- Markdown ドキュメント更新時のルール
- Windows + PowerShell 環境での注意点
- このリポジトリに定義されているカスタムエージェントの役割

毎回プロンプトに細かい前提を書かなくても、リポジトリに共通ルールを置いておくことで、Copilot にプロジェクトの作法を伝えやすくなります。

### カスタムスキル

`.github/skills/` には、特定の作業を行うときに参照する手順書のようなものが定義されています。

| スキル | 内容 |
|--------|------|
| `feature-lifecycle` | 仕様策定から実装、レビュー、テスト、ドキュメント更新までの流れ |
| `github-flow` | feature ブランチ、Draft PR、レビュー、マージまでの開発フロー |
| `clean-architecture-guide` | Modular Monolith + Clean Architecture の設計ルール |
| `dotnet-best-practices` | .NET / C# 実装時のベストプラクティス |
| `bunit` | Blazor コンポーネントテストの書き方 |
| `documentation-guide` | Markdown ドキュメント更新時のルール |

スキルを使うことで、「この作業ではどの手順やルールに従うべきか」を Copilot に明示できます。チームで開発する場合は、設計ルール、テスト方針、PR 作成手順などをスキルとして整備しておくと、作業の進め方を揃えやすくなります。

### カスタムエージェント

`.github/agents/` には、開発ライフサイクルを役割ごとに分けたカスタムエージェントが定義されています。

| エージェント | 役割 |
|--------------|------|
| Orchestrator | 全体の進行管理、状態管理、各エージェントへの委譲 |
| Product Manager | 要件整理、仕様書 Issue の作成 |
| Architect | 設計レビュー、完成判断 |
| Developer | 実装、ビルド、テスト、PR 作成 |
| Reviewer | 仕様・コードレビュー |
| Tester | E2E テスト、バグ報告 |
| Documentation | ドキュメント更新 |

通常のエージェント モードでは、1 つの Agent に対して調査、計画、実装、検証を順番に依頼します。一方で、カスタムエージェントを使うと、要件整理、設計、実装、レビュー、テスト、ドキュメント更新といった役割を分けて考えられます。

### 発展: Orchestrator による自律開発ワークフロー

このリポジトリには、`feature-lifecycle` スキルと `Orchestrator` エージェントを組み合わせた、より発展的な開発ワークフローも定義されています。

ただし、Visual Studio 2026 の GitHub Copilot は、現時点ではサブエージェント呼び出しをサポートしていません。そのため、`.github/agents/` に定義されている複数のエージェントを Orchestrator から呼び分けるワークフローは、Visual Studio 2026 上では利用できません。

この仕組みは、VS Code や GitHub Copilot CLI など、サブエージェント呼び出しを扱える環境で利用することを想定しています。Orchestrator は `feature-lifecycle` に書かれた流れに従って、役割別のカスタムエージェントに作業を委譲します。

| フェーズ | 担当エージェント | 内容 |
|----------|------------------|------|
| 仕様策定 | Product Manager / Reviewer | 要件を仕様書 Issue に整理し、レビューする |
| 設計・計画 | Architect / Developer / Documentation / Reviewer | 設計方針、実装計画、ドキュメント骨子を作り、レビューする |
| 実装 | Developer | GitHub Flow に従って実装、ビルド、テスト、Draft PR 作成を行う |
| 検証 | Tester / Reviewer / Architect | E2E テスト、コードレビュー、完成判断を行う |
| ドキュメント追従 | Documentation | 実装差分に合わせて docs を更新する |

本編では、この流れを人間が手順として分けて実行しました。発展的な環境では、その進行管理自体を Orchestrator に任せることで、要件定義から PR 作成、レビュー、テスト、ドキュメント更新までを一連のライフサイクルとして扱えます。

### ここから学べること

カスタム指示、カスタムスキル、カスタムエージェントは、Copilot に「このプロジェクトではどう進めるべきか」を伝えるための仕組みです。

個人で Copilot を使う場合は、プロンプトに毎回指示を書くだけでも十分なことがあります。一方で、チーム開発では次のような情報をリポジトリに置いておくと、Copilot の回答や実装方針を揃えやすくなります。
...
つまり、Copilot を単なるコード生成ツールとして使うだけでなく、チームの開発プロセスや設計ルールを理解した開発支援ツールとして使うための準備が、この `.github` 配下の定義です。
